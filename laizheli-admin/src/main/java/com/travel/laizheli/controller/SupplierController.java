...
public class SupplierController {
    ...
    public Result getCount(){
        ...
        // 获取商品数量
        int supplierCount = supplierService.getCount();


        return Result.success(supplierCount,"供应商总量获取成功");
    }
    @GetMapping("/state")
    public Result updateState(@RequestParam(value = "supplierId")String id,
                              @RequestParam(value = "supplierState")Integer state){
        int i = supplierService.updateState(state,id);
        System.out.println(i);
        //int i = qualificationService.updateState(representIdcard,state);
        if(i == 1)
            return Result.success(null,"成功修改状态");
        return Result.failed("修改状态失败");
    }
//    @GetMapping("/state")
//    public Result updateState(@RequestParam("supplierId")Integer supplierId,@RequestParam("state") int state){
//        Supplier<Supplier> goods = supplierService.
...
//    public String getUserList(Supplier supplier) {
//
//        List<Supplier> suppliers = supplierService.getAllSupplier(supplier.getId());
//        HashMap<String, Object> res = new HashMap<>();
//
//        res.put("data", suppliers);
